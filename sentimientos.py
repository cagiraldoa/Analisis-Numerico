...
def emociones_(image_):

    emotions = ["Miedo", "Felicidad", "Neutral", "Tristeza"] #lista de sentimientos
    ...
    make_sets(emotions, clahe, detector, predictor, image_)

    probam1 = np.zeros((4,10))
    probam2 = np.zeros((1,4))
 
    accur_lin = []
    print("Haciendo analisis y comprobación de sentimientos en tu foto... Tardara un poco")
 
    for i in range(0,10):
        #print("Making sets %s" %i) #hace un muestreo aleatorio 80/20%
        training_data, training_labels, prediction_data, prediction_labels = make_sets(emotions, clahe, detector, predictor, image_)
...
def make_sets(emotions, clahe, detector, predictor, image_):
  training_data = []
  training_labels = []
  prediction_data = []
  prediction_labels = []
  ...
  for emotion in emotions:
      training, prediction = get_files(emotion, image_)
      #Append data to training and prediction list, and generate labels 0-7
      for item in training:
          image = cv2.imread(item) #open image
          gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) #convertimos a escala de grises
          clahe_image = clahe.apply(gray)
          landmarks_vectorised = get_landmarks(clahe_image, detector, predictor)
